...
public class Board {
    ...
    public Board() {
        ...
        for (int i = 0; i < board.length; i++) {
            if (i < 6) {
                board[i] = new Hole(6, counter++, 1, opposite--);
            } else if (i == 6) {
                board[i] = new Hole(0, counter++, 1, -1);
                counter = 0;
                opposite = 5;
            } else if (i < 13) {
                board[i] = new Hole(6, counter++, 2, opposite--);
            } else {
                board[i] = new Hole(0, counter++, 2, -2);
            }
        }
    }
    ...
    public boolean moveStones(int position, int team) {
        ...
        return false; //without bonus move
    }
